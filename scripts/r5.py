...
class CombinedSlopeWalk(Node):
    def __init__(self):
        ...
        # 上台阶步态参数（速度更快）
        self.declare_parameter("stage1_forward_speed", 0.30)   # 上台阶速度 0.30
        self.declare_parameter("stage1_step_height", 0.14)
        ...
        # 快速行走步态参数（速度提升，实际还会×1.2）
        self.declare_parameter("stage2_forward_speed", 0.40)   # 0.30→0.40，实际速度 0.48 m/s
        ...
        self.declare_parameter("extra_dest_y", 13.31468)
        self.declare_parameter("extra_forward_speed", 0.45)   # 0.30→0.45，最后直线更快
        self.declare_parameter("extra_step_height", 0.12)
        self.declare_parameter("extra_body_height", 0.30)
        ...
        self.get_logger().info("=== COMBINED WALK + SLOPE + EXTRA WALK (FAST) ===")
